 ...
 * folder and set a plugin in pom.xml
 * 
 * @author Yong Zhu
 * @since 2.0.4
 */
public class Txt {
	...
	public static String classToString(Class<?> clazz) {
		...
		String classText = GsgStrUtils.substringBetween(javaSourceCode, "public static class " + thisPublicStaticClassName,
				"*/");
		if (GsgStrUtils.isEmpty(classText))
			throw new RuntimeException("Can not find text between \"public static class " + thisPublicStaticClassName
					+ " and end tag \"*/\"");
		String s = GsgStrUtils.substringAfter(classText, "/*-");
