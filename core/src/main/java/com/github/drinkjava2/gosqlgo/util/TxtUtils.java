 ...
 * @author Yong Zhu
 */
public abstract class TxtUtils {// NOSONAR
	...
	public static String getJavaSourceCode(Class<?> clazz, String encoding) {
		...
		String classPathName = GsgStrUtils.substringBefore(clazz.getName(), "$");// aa.bb.Cc
		classPathName = "/" + GsgStrUtils.replace(classPathName, ".", "/");// /aa/bb/Cc
		String fileName = classPathName + ".java";// /aa/bb/Cc.java
		InputStream inputStream = null;
		try {
			inputStream = TxtUtils.class.getResourceAsStream(fileName);
			if (inputStream == null) {// Not found, it means in eclipse
				File file = new File(clazz.getResource(classPathName + ".class").getFile());
				String absPath = file.getAbsolutePath();
				absPath = GsgStrUtils.replace(absPath, "\\", "/");
				String projectFolder = GsgStrUtils.substringBefore(absPath, "/target/");
